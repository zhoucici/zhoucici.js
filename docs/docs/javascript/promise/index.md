...
{
  "title": "手搓Promise",
  'description': "手写Promise，以及Promise的使用方式、api等等"
}
---
# 手搓Promise

面试的时候老是被问Promise这个那个的,现在直接手搓一个Promise,让面试官看看你的实力。

[`promisesA+规范`](https://promisesaplus.com/)镇楼,下文我们叫他：[`老A`](https://promisesaplus.com/)
...
[`老A`](https://promisesaplus.com/)对Promise有规定：
...
```
## 3. then方法
[`老A`](https://promisesaplus.com/)规定:Promise有一个叫做then的方法，里面有两个参数：onFulfilled,onRejected,成功有成功的值，失败有失败的原因
...
```
现在基本可以实现简单的同步代码，对于带setTimeout的异步代码还是没有用
## 4. 解决异步实现

当then时state还是pending等待状态 我们就需要在then调用的时候，将成功和失败存到各自的数组，一旦reject或者resolve，就调用它们
...
1. 为了达成链式，我们默认在第一个then里返回一个promise。[`老A`](https://promisesaplus.com/)规定了一种方法，就是在then里面返回一个新的promise,称为promise2：`promise2 = new Promise((resolve, reject)=>{})`
    - 将这个promise2返回的值传递到下一个then中
    - 如果返回一个普通的值，则将普通的值传递给下一个then中
  
2. 当我们在第一个then中`return`了一个参数（参数未知，需判断）。这个return出来的新的promise就是onFulfilled()或onRejected()的值
   
[`老A`](https://promisesaplus.com/)则规定onFulfilled()或onRejected()的值，即第一个then返回的值，叫做x，判断x的函数叫做resolvePromise
...
[`老A`](https://promisesaplus.com/)规定了一段代码，让不同的promise代码互相套用，叫做resolvePromise
